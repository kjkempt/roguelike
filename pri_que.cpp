...
/*
 ...
 and there are several small changes made by myself but the structure and majority of the code was done
...
void dijkstra(struct Graph* graph, int src, dungeonGrid **grid, int mode, int parent[])
{
    ...
    //int parent[V];      //keeps track of the path to the source
    ...
    if(mode == 0)
    {
    //printArrTun(dist, V, grid);
    }
    if(mode == 1)
    {
        //printArrParent(parent, V);
        //printArrNonTun(dist, V, grid);
    }
